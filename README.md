...
```bash
pip install pangolin           # or: pip install -e . from a clone of this repo, for local dev
pangolin init my-project        # scaffold config/, pipelines/, custom/, .env
pangolin init my-project -d     # also scaffold the Docker deployment stack (docker/, docker-compose.yml, Makefile)
cd my-project
...
│       └── docker/, docker-compose.yml, Makefile  (only with -d/--dockerization)
...
└── docker/                     # Dockerfile, Caddyfile, prefect_manifest.yaml (with -d)
```
